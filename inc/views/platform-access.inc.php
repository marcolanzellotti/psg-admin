...
if (isset($_GET['verify'])) {
    ...
    $qUser = $pCon->query("SELECT * FROM users WHERE id=$user");
...
$qUsers = $pCon->query("SELECT * FROM users WHERE verified=0 $completion GROUP BY email ORDER BY id DESC");

?>
<div class="container" id="updates">
    <div class="scroll-x">
        <h5>Acessos plataforma (<?= $qUsers->num_rows ?> pendentes) </h5>
        <form action="?view=platform_access" method="POST">
            <div class="row">
        ...
        <table class="striped small">
            ...
            ?>
                <tr>
                    ...
                    <td>
                        <a target="_blank" href="https://api.whatsapp.com/send?phone=<?= $user['phone'] ?>" style="padding:3px 10px;background-color:limegreen;color:white;border-radius:3px;">Whatsapp</a>
                    </td>
                    <td>
                        <a href="?view=platform_access&verify=<?= $user['id'] ?>" style="padding:3px 10px;background-color:#1a9af0;color:white;border-radius:3px;">Liberar</a>
                    </td>
                    <td>
                        <a href="?view=platform_access&delete=<?= $user['id'] ?>" class="red" style="padding:3px 10px;color:white;border-radius:3px;">Excluir</a>
