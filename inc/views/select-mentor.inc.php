 ...
 <div class="container  scroll-x" id="select_mentor" style="padding-top: 10em;"> <!--  TODO: FIX THAT -->
     <form action="" method="POST">
         <div class="row">
             <div class="col m6 s12 input-field">
                 <input type="text" name="search_select_co_author" required>
     ...
     <table class="striped small">
         ...
         <?php
            ...
            $search = E($_POST['search_select_co_author']);
            $searchCompletion = (isset($_POST['search_select_co_author'])) ? "AND (name LIKE '%$search%' OR phone like '%$search%')" : "";

            //var_dump($_SESSION);

            //echo "SELECT * FROM habits  WHERE job <> '' AND (author='$loggedMentor[name]' $completion) $searchCompletion ORDER BY id DESC";
            $query = "SELECT * FROM habits  WHERE job <> '' AND (author='$loggedMentor[name]' $completion) $searchCompletion ORDER BY id DESC";
            ...
            foreach ($habits as $data) {
                if ($_SESSION['mentor'] == "master") {
                    $completion = ($loggedMentor['name'] == "Master") ? " OR author_id=7 " : "";

                    $mentors = $con->query("SELECT * FROM mentors WHERE is_co_author=1 AND author_id=$loggedMentor[id]  $completion");
            ...
            ";
                    ...
                    $qtdDia = explode("/", $data['create_date']);

                    $date = $qtdDia[2] . "-" . $qtdDia[1] . "-" . $qtdDia[0];

                    $newDate = new DateTime($date);

                    $diferenca = $dataAtual->diff($newDate);
                ...
                //$subDate = $dataAtual - $newDate;

                //$resultado = date_diff($newDate, $dataAtual);

                //var_dump($subDate);

                //if()
                //var_dump($qtdDia[2] . "-" . $qtdDia[1] . "-" . $qtdDia[0]);
