...
<div class="container  scroll-x" id="select_mentor" style="padding-top: 10em;"> <!--  TODO: FIX THAT -->
    ...
    <table class="striped small">
        ...
        $query = "SELECT * FROM habits WHERE 1 /*OR (habits.mentor='$loggedMentor[username]' OR habits.co_author=$loggedMentor[id] OR habits.mentor IN (SELECT username FROM mentors WHERE co_author_id=$loggedMentor[id])) */  $completion $searchCompletion ORDER BY id DESC";
        // var_dump($query);
        ...
        foreach ($habits as $data) {
            ...
            $mentors = $con->query("SELECT * FROM mentors WHERE 1  AND is_co_author=0 $completion");
            ...
            if (!$data['co_author']) {
                $coAuthor = "<b style=\"color:red;\">Não selecionado</b>";
            } else {
                $qCoAuthor = $con->query("SELECT * FROM mentors WHERE id=$data[co_author]");

                $coAuthor = $qCoAuthor->fetch_assoc()['name'];
            }
    ...
    <ul class="pagination">
        ...
        ?>
            <li <?= $_GET['page'] == $counter ? " class=\"active\"" : "" ?>><a href="?view=select_mentor_all&page=<?= $counter ?>"><?= $counter ?></a></li>
        <?php
        }
        ?>
    </ul>
