...
$qCustomer = $pCon->query("SELECT * FROM users WHERE id=$id");
...
if (issetPostFields(["name", "email", "phone"])) {
    ...
    $qUpdateCustomer = $pCon->query("UPDATE users SET name='$name', email='$email', phone='$phone' WHERE id=$id");

    if ($qUpdateCustomer) {
        $qCustomer = $pCon->query("SELECT * FROM users WHERE id=$id");
        $customer = $qCustomer->fetch_assoc();
        echo "<script>document.location.href=\"painel.php?view=platform_users\"</script>";
    }
}
?>
<div class="container">
    <h5>Editar Usuário Plataforma</h5>
    ...
    <form method="POST" action="">
        ...
        <div class="row">
            <div class="col s12">
                ...
                <a href="?view=platform_users" class="btn grey">Voltar <i class="material-icons">undo</i></a>
            </div>
